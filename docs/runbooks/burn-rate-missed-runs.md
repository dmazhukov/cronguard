...
- **CronJobMonitor has `spec.timeZone` mismatched with `CronJob.spec.timeZone`** — operator computes missed runs against `spec.timeZone`, but kube-controller-manager fires against `CronJob.spec.timeZone`. Check both:
  ...
  ```
  The same check covers a **daylight-saving transition**. On the fall-back day a daily job **whose slot falls inside the repeated hour** has *two* expected runs — the wall clock passes through that hour twice — so one suppressed run is a 50% burn against that day and can trip the fast window. Which hour repeats is zone-specific: 01:00 in the US, 02:00 in the EU, so the same expression is doubled in one zone and untouched in another. On the spring-forward day a job whose only slot falls in the skipped hour has *no* expected run, so no burn can be recorded at all. Both are correct by definition; see the daylight-saving section of the `internal/schedule` package doc. Do not silence the fall-back case — a suppressed execution is real, and it is worth a look once a year.
...
- `cronguard_missed_runs` — gauge of the current consecutive-miss streak, counted from the later of the last run this operator observed and the CronJob's own `status.lastScheduleTime`. (Not "since last success": the streak resets on any observed start, including a failed one — failures are the `ExecutionHealthy` axis.) Companion to the `_total` counter; useful for "how bad is it right now."

  Because the counter accrues the *positive delta* of that streak, a miss streak that both begins and ends while the operator is not running is not counted — the operator only ever sees the endpoints. Neither `CronJob.status.lastScheduleTime` nor `lastSuccessfulTime` records skipped slots, so this is not recoverable from the Kubernetes API. After a downtime of D, up to `floor(D / period)` real misses can go uncounted if any run was *scheduled* near the end of the window — the floor advances on a successful Job creation, not on Job success. See [ADR 0001](../adr/0001-cronjob-status-as-missed-run-floor.md).
