...
- **CronJobMonitor has `spec.timeZone` mismatched with `CronJob.spec.timeZone`** — operator computes missed runs against `spec.timeZone`, but kube-controller-manager fires against `CronJob.spec.timeZone`. Check both:
  ...
  ```
  The same check covers a **daylight-saving transition**. On the fall-back day a daily job has *two* expected runs — the wall clock passes through the slot twice — so one suppressed run is a 50% burn against that day and can trip the fast window. On the spring-forward day a job whose only slot falls in the skipped hour has *no* expected run, so no burn can be recorded at all. Both are correct by definition; see the daylight-saving section of the `internal/schedule` package doc. Do not silence the fall-back case — a suppressed execution is real, and it is worth a look once a year.
