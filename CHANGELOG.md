...
- **`@every` schedules are rejected wherever they come from.** A CEL rule already rejected them on `CronJobMonitor.spec.schedule`, but the reconciler falls back to `CronJob.spec.schedule`, which Kubernetes accepts. `@every` is an interval with no wall-clock grid, so drift is always ~0 and missed runs are uncountable; such a monitor now reports `InvalidSchedule` instead of publishing meaningless numbers.
