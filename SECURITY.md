...
Only the latest minor release line receives security updates — today that is `0.4.x`. A fix ships as a new patch release with a rebuilt image; older lines are not patched. Release artifacts carry GitHub build attestations and a `checksums.txt`, the image and OCI chart cosign signatures, and charts released after v0.4.1 a PGP provenance file; see [docs/distribution.md](docs/distribution.md#verifying-artifacts) for the verification commands.
